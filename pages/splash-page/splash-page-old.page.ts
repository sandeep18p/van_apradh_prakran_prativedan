...
export class SplashPagePage implements OnInit {
  ...
  async getNgRokURL() {
    ...
    this.apiService.takeNGRokURL().subscribe(
      async (response) => {
        if (response.response.status === "true") {
          ...
          if (value) {
            ...
            if (userData.emp_id.toString() === response.data.tester_user_id) {
              await Preferences.set({ key: 'ngrok_url', value: response.data.forest_complain_ngrok_url_testing });
            } else {
              // Testing URL means Testing Mode without testing URL means production mode
              await Preferences.set({ key: 'ngrok_url', value: response.data.forest_complain_ngrok_url });
              // await Preferences.set({ key: 'ngrok_url', value: "https://adf5-2409-4043-2e18-634-a002-e4a3-47c0-f7e9.ngrok-free.app/api/ForestComplainMonitoringSystem/" });
            }
