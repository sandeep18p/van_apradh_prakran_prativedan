...
import { addCircleOutline, arrowBack, calendarOutline, cameraOutline, calculatorOutline, checkmarkCircle, checkmarkCircleOutline, chevronDown, chevronUp, close, closeCircle, closeCircleOutline, createOutline, documentTextOutline, imagesOutline, informationCircleOutline, leafOutline, locationOutline, micCircleOutline, peopleOutline, trashOutline } from 'ionicons/icons';
...
@Component({
  selector: 'app-submit-van-apradh-prakaran-by-ra-for-beat-nirikshan',
  templateUrl: './submit-van-apradh-prakaran-by-ra-beat-nirikshan-por.component.html',
  styleUrls: ['./submit-van-apradh-prakaran-by-ra-beat-nirikshan-por.component.scss'],
  imports: [NgSelectModule, IonicModule, FormsModule, CommonModule, TableModule],
})
export class SubmitVanApradhPrakaranByRaForBeatNirikshanComponent implements OnInit {
  ...
  listOfjaptiSaman: (JaptSamanItem & { can_delete: boolean, compartment_number: string })[] = [];
  ...
  // Image preview/cropper (RajiNama photo only)
  ...
  parseValueForCompartmentOptionAndNumber(text: string) {
      ...
      (response) => {
        ...
        if (response.response.code === 200) {
          ...
          if (response.complainData && response.complainData.length > 0) {
            ...
            console.log(this.listOfWitness, ' listOfWitness');
            ...
            const resultCompartmentNumber = this.compartmentNumber.split(",").map(item => item.trim());


            this.listOfDharaDataNew;

            resultCompartmentNumber.forEach(item => {

              let compartmnetNumberAndoption = this.parseValueForCompartmentOptionAndNumber(item);

              const code = compartmnetNumberAndoption.code;
              const type = compartmnetNumberAndoption.type;

              this.clipboardCompartment.push({ name: code, option: type as 'RF' | 'PF' | 'OA' | null });
  ...
  addChattaInfo() {
    this.listOfChattaDetail.push({
      ...
      if_not_yogya_then_reason: '', is_janch_karta_entry: false, can_delete: true, compartment_number: ''
    });
  }
  ...
  addThunthInfo() {
    ...
    this.listOfThunthDetail.push({
      ...
      is_janch_karta_entry: false, site_quality: 0, can_delete: true, compartment_number: ''
    });
  }
      ...
      buttons: [
        {
          ...
          handler: async () => {
            ...
            if (image.dataUrl) {
              this.openRajiNamaImagePreviewModal(image.dataUrl);
            }
          }
        },
        {
  ...
  openRajiNamaImagePreviewModal(imageDataUrl: string) {
  ...
  approveRajiNamaImage() {
    ...
    this.closeRajiNamaImagePreviewModal();
  }
  ...
  closeRajiNamaImagePreviewModal() {
    if (this.cropperInstance) {
      try {
        this.cropperInstance.destroy();
      } catch (e) {
  ...
  submitPativedan() {
    ...
    ;
    if (this.actual_crime_date != "") {
      this.actual_crime_date = this.formatDateStringIntoYYYYMMDD(this.actual_crime_date);
    } else {
      this.actual_crime_date = "";
    }

    formData.append('actual_crime_date', this.actual_crime_date);
    ...
    if (this.isShowEditDharaBox) {
      ...
      if (this.clipboardCompartment.length > 0) {
        ...
        if (commaSeparatedCompartment === "" || commaSeparatedCompartment === null) {
          formData.append('compartment_number', "0");
        } else {
          formData.append('compartment_number', commaSeparatedCompartment);
        }

        // Collect all compartment options and join with comma
        const compartmentOptions = this.clipboardCompartment
          .filter(c => c.option !== null)
          .map(c => c.option)
          .join(', ');
        formData.append('compartment_option', compartmentOptions || '');
      } else {
        formData.append('compartment_number', "0");
        formData.append('compartment_option', '');
      }
    ...
    for (let i = 0; i < this.listOfJaptVahanDetail.length; i++) {
      ...
      console.log(row);
      // row.malik_ka_name="skd";
      // row.malik_k_father_ka_name="skd";
      // row.pata='romalik_k_father_ka_name';
  ...
  addBanshInfo() {
    this.listOfBanshDetail.push({
      ...
      lambai: '', golai: '', ghan_meter: '', nag: '', dar: '', total_cost: '', if_other_then_detail: '', motai: '',
      unchai: '', kasth_halat: 0, is_yogya_to_parivahan: '', /// 0-no,1-yes
      if_not_yogya_then_reason: '', is_janch_karta_entry: false, is_dar_editable: true, can_delete: true, compartment_number: ''
    });
  }
  ...
  async onCrimTypeChange() {
    ...
    // If this change would clear/affect selected dhara, confirm first.
    ...
    // Persist as the last confirmed value
    ...
    dharaArray.forEach((dhara, index) => {
      ...
      this.dharaNewGroups.forEach(group => {
        group.items.forEach(item => {
          ...
          let listDhara = (item as any).dhara || (item as any).adhara;
  ...
  buildDharaNewGroups() {
    ...
    this.selectedCompartmentOption =
      selectedOptions.size > 0
        ? ([...selectedOptions][0] as 'RF' | 'PF' | 'OA')
        : null;
  ...
  private captureEditBackup(option: '1' | '2' | '3' | '4') {
    if (option === '1') {
      this.editBackup1 = {
        ...
        selectedCompartmentNames: this.clone(this.selectedCompartmentNames),
  ...
  cancelSectionEdit(option: '1' | '2' | '3' | '4') {
    if (option === '1' && this.editBackup1) {
      ...
      this.selectedCompartmentNames = this.clone(this.editBackup1.selectedCompartmentNames);
    ...
    modal.onDidDismiss().then(async (result) => {
      if (result.data?.confirmed) {
        if (optionToEdit === "1" || optionToEdit === "2" || optionToEdit === "3" || optionToEdit === "4") {
          this.captureEditBackup(optionToEdit);
        }

        if (optionToEdit === "1") {
          this.isShowEditDharaBox = true;
          // Rebuild/preselect dhara when opening edit UI.
          // Preselect logic reads `comingComplaintData.crime_dhara` inside `buildDharaNewGroups()`.
          setTimeout(() => {
            this.selectedDharaNewIds.clear();
            this.selectedDharaNewItems = [];
            this.selectedDharaItemsArray = [];
            ...
            if (this.selectedCrimType && Array.isArray(this.listOfDharaDataNew) && this.listOfDharaDataNew.length > 0) {
              this.buildDharaNewGroups();
            }
          }, 0);
        } else if (optionToEdit === "2") {
  ...
  @ViewChild('compartmentDropdownWrap', { static: false })
  compartmentDropdownWrap?: ElementRef<HTMLElement>;

  @HostListener('document:click', ['$event'])
  onDocumentClickForCompartmentDropdown(event: MouseEvent) {
    if (!this.isCompartmentDropdownOpen) return;

    const wrap = this.compartmentDropdownWrap?.nativeElement;
  ...
  async onCompartmentOptionChangeForCompartment(compartmentName: string, option: 'RF' | 'PF' | 'OA') {
    ...
    if (!compartment) {
      ...
      // If changing an existing option, confirm (because it can reset/affect selected dhara)
      const prev = compartment.option;
      if (prev && prev !== option) {
        const ok = await this.confirmYesNo(
