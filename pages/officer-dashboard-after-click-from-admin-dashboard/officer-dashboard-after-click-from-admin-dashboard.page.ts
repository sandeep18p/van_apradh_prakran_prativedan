...
import { add, addOutline, addSharp, arrowBack, calendarOutline, checkmarkCircleOutline, closeCircleOutline, cloudOfflineOutline, downloadOutline } from 'ionicons/icons';
...
import { ComplainDetails, TotalPORCountResponseModal } from './GetDashboardResponse.model';
...
@Component({
  selector: 'app-officer-dashboard-after-click-from-admin-dashboard',
  templateUrl: './officer-dashboard-after-click-from-admin-dashboard.page.html',
  styleUrls: ['./officer-dashboard-after-click-from-admin-dashboard.page.scss'],
  standalone: true,
...
export class OfficerDashboardAfterClickFromAdminDashbaordPage implements OnInit {
  ...
  constructor(
    ...
    addIcons({ downloadOutline, cloudOfflineOutline, addOutline, checkmarkCircleOutline, closeCircleOutline, arrowBack, calendarOutline });
  }
  ...
  async onMenuItemClick(page: string) {
    ...
    if (page === "logout") {
      ...
      if (page === "janch_awadhi_badhane_hetu_kiye_gaye_awedan") {
  ...
  comingFromDashboardPage!: TotalPORCountResponseModal;

  async getLoginedOfficerName() {
    ;
    const state = history.state;

    this.comingFromDashboardPage = state.data as TotalPORCountResponseModal;

    this.isCCF = state.isCCF;
    this.isDFO = state.isDFO;

    // If coming from admin dashboard table, carry date range forward
    if (state?.startDate) {
    ...
    if (this.isCCF) {
      this.toolbarTitle = "वृत्त का नाम : " + this.comingFromDashboardPage.circleName;
      this.selectedCircleId = this.comingFromDashboardPage.circleId;
    } else {
      this.toolbarTitle = "वृत्त का नाम : " + this.comingFromDashboardPage.circleName + ", वनमंडल का नाम : " + this.comingFromDashboardPage.divisionName;
      this.selectedCircleId = this.comingFromDashboardPage.circleId;
      this.selectedDivisionId = this.comingFromDashboardPage.divisionId;
    }
    ...
    if (value) {
      ...
      this.loginedOffierDesignationId = userData.designation_id.toString();
  ...
  setDefaultDates() {
    // Respect any forwarded date range (e.g. from admin dashboard)
    ...
    // const todayLocal = new Date(now.getTime() - offset);
    // this.endDate = todayLocal.toISOString().split('T')[0];

    // const twoMonthsAgo = new Date(now.getTime() - offset);
